...
export async function getProfile(userId: string) {
  ...
  const tickets = await db
    .collection<AuthTicketDoc>("tickets")
...
export async function createTicketBooking(
  ...
  const doc: Omit<AuthTicketDoc, "_id"> = {
  ...
  const result = await db.collection<AuthTicketDoc>("tickets").insertOne(doc as AuthTicketDoc);
...
function toTicketBooking(ticket: AuthTicketDoc): TicketBooking {
  return {
